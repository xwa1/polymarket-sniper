...
export async function GET(req: NextRequest) {
  ...
  const now = new Date();
  const params = new URLSearchParams({ active: "true", closed: "false", limit: "100", order: "end_date_asc" });
  try {
    const res = await fetch(`${GAMMA_BASE}/markets?${params}`, { headers: { Accept: "application/json" }, cache: "no-store" });
    if (!res.ok) return NextResponse.json({ error: `Gamma API error: ${res.status}` }, { status: 502 });
    const raw = await res.json();
    const markets = raw.map((m: any) => {
      let prices: number[] = [];
      try { prices = JSON.parse(m.outcomePrices ?? "[]").map((p: any) => parseFloat(p)); } catch { if (m.lastTradePrice) prices = [m.lastTradePrice]; }
      ...
      return { id: m.id, question: m.question, slug: m.slug, category: m.category ?? "", endDate, daysLeft, bestProb, bestOutcomeName, volume: volumeNum, volume24hr: m.volume24hr ?? 0, bestBid: m.bestBid ?? 0, bestAsk: m.bestAsk ?? 0, spread: m.spread ?? 0, lastTradePrice: m.lastTradePrice ?? 0, oneDayPriceChange: m.oneDayPriceChange ?? 0 };
    }).filter((m: any) => m.bestProb >= minProb && m.volume >= minVol && m.daysLeft > 0 && m.daysLeft <= maxDays).sort((a: any, b: any) => b.bestProb - a.bestProb);
