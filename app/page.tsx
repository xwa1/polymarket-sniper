    ...
    try {
      ...
      for (const market of politicalMarkets) {
        ...
        detected.push({
          ...
          lastHourVol: market.volume24hr / 8,
          alertLevel,
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.5rem", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: "#4b5563" }}>
            Political markets with abnormal hourly volume spikes
            {lastScan && <span style={{ color: "#374151" }}> · Last scan: {lastScan}</span>}
          </p>
        </div>
      ...
      <div style={{ background: "rgba(99,102,241,0.05)", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 12, padding: "1rem", marginBottom: "1.5rem" }}>
        ...
        <p style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.6 }}>
          We scan active political markets every 5 minutes. For each market we estimate the daily average volume and compare it against the last 24h volume. A spike of 3x or more — with no apparent news — may indicate insider positioning before a major announcement.
        </p>
        <div style={{ display: "flex", gap: 16, marginTop: 10, flexWrap: "wrap" }}>
          {[
            { label: "MODERATE", threshold: "2.5x – 5x", color: "#fcd34d" },
            { label: "HIGH", threshold: "5x – 10x", color: "#fb923c" },
            { label: "EXTREME", threshold: "> 10x", color: "#f87171" },
      ...
      {scanning && (
        <div style={{ textAlign: "center", padding: "3rem 0", color: "#4b5563" }}>
          <div style={{ fontSize: 13 }}>Scanning political markets for anomalies...</div>
        </div>
      )}
      ...
      {!scanning && signals.length === 0 && (
        <div style={{ textAlign: "center", padding: "3rem 0" }}>
          ...
          <div style={{ fontSize: 14, color: "#4b5563" }}>No insider signals detected right now. Markets look quiet.</div>
        </div>
      )}
            ...
            return (
              <div key={s.market.id} style={{ background: ac.bg, border: `1px solid ${ac.border}`, borderRadius: 14, padding: "1rem 1.25rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <span style={{ fontSize: 9, fontWeight: 800, color: ac.text, border: `1px solid ${ac.border}`, borderRadius: 4, padding: "2px 8px", letterSpacing: "0.1em" }}>
                    ⚡ {alertLabels[s.alertLevel]}
                  </span>
                  <span style={{ fontSize: 11, color: ac.text, fontFamily: "monospace", fontWeight: 700 }}>
                    {s.spikeRatio.toFixed(1)}x spike
                  </span>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(90px, 1fr))", gap: 10, marginBottom: 12 }}>
                  <div>
                    ...
                    <SpikeChart spikeRatio={s.spikeRatio} />
                  </div>
                  {[
  ...
  return (
    <div className="mcard" style={{
      ...
      <div>
        ...
        <div style={{ fontSize: 9, color: "#4b5563" }}>approx.</div>
      </div>
